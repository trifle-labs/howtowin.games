// Fox and Geese — 33-point cross board. One fox (AI), four geese (you).
// Geese (you) move forward or sideways one step. Fox moves any direction one
// step, and captures by jumping over an adjacent goose into an empty square
// (chain jumps allowed; captures NOT mandatory here for simplicity).
// You win if the fox has no legal move; fox wins if geese can no longer pen
// the fox (≤3 geese remaining).
...
export function create(canvas) {
  ...
  // Cross-shaped 7×7 with the four 2×2 corners removed. Build coords.
  const CELLS = [];
  const isCell = (r, c) => {
  ...
  let board, turn, winner, sel, mustJumpFrom;
  function newGame(){
    board = new Array(CELLS.length).fill('.');
    // geese at top two rows (r=0,1) of the cross
    for (let i = 0; i < CELLS.length; i++){ const [r, c] = CELLS[i]; if (r <= 1) board[i] = 'g'; }
    // fox at center
    board[idxOf.get('3,3')] = 'f';
    turn = "you"; winner = null; sel = -1; mustJumpFrom = -1;
  }
  newGame();
  ...
  function gooseMoves(){
    ...
    for (let i = 0; i < CELLS.length; i++) if (board[i] === 'g'){
      for (const n of neighbours(i)){
        ...
        // geese can move sideways or DOWN (towards bottom). Not upward (away from fox's pen).
        if (n.dr < 0) continue;
        out.push([i, n.idx]);
      }
    }
  ...
  function foxMoves(fromIdx){
    // returns plain moves and jump chains (one-step jumps; user-friendly version)
    const out = [];
  ...
  function aiMove(){
    ...
    if (geeseLeft <= 3){ winner = "ai"; draw(); return; }
    turn = "you"; draw();
  }
  ...
  function draw(){
    ...
    _fit(ctx, "Fox and Geese — geese (you) push the fox toward the bottom edge", W/2, 14, W - 8);
    ...
    for (let i = 0; i < CELLS.length; i++){
      ...
      if (v === 'g'){
        ...
        ctx.fillText("G", rc.x + rc.w/2, rc.y + rc.h/2); ctx.textBaseline = "alphabetic";
        ctx.font = "13px sans-serif";
      }
    }

    if (winner) statusEl.textContent = winner === "you" ? "you penned the fox — you win!" : "Fox eats enough geese — AI wins";
  ...
  function onClick(e){
    ...
    // valid move? down or sideways and empty
