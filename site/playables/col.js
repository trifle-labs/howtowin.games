// Col — partisan map-colouring game. Played on a small graph (a 5-node cycle
// with one inner cross-edge). On your turn pick an empty node and colour it
// your colour (you=Blue, AI=Red); no two adjacent nodes may share a colour.
...
export function create(canvas) {
  ...
  // 6 nodes arranged in 2 rows (hexagonal-ish); adjacency below.
  const N = 6;
  const COORDS = (() => {
  ...
  function legal(n, side){
    ...
    for (let i=0; i<N; i++){
      if (n[i] !== ".") continue;
      let ok = true;
      for (const j of NBR[i]) if (n[j] === my){ ok = false; break; }
      if (ok) out.push(i);
    }
    return out;
  }
  ...
  function draw(){
    ...
    ctx.fillText(`Col — you = Blue, AI = Red; no two same-colour neighbours`, size/2, 22);
    // edges
    ...
    if (winner) statusEl.textContent = winner === "you" ? "you win — AI cannot colour!" : "AI wins — you cannot colour!";
    else statusEl.textContent = turn === "you" ? "click a green node to colour it blue" : "AI thinking…";
  }
