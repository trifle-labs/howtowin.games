// Eleven Men's Morris — standard 24-point morris board, 11 pieces per side.
// Otherwise identical to Nine Men's Morris (placement then movement; mills of
// 3 remove an enemy piece; lose at <3 pieces or no legal move).
...
export function create(canvas) {
  ...
  function draw(){
    ...
    ctx.fillText(`Eleven Men's Morris — ${phase === "place" ? `placement (you ${placed.you}/${NUM}, ai ${placed.ai}/${NUM})` : "movement"}`, W/2, 22);
  ...
  return {
    solve(){
      if (winner || turn !== "you") return;
      if (captureMode){
        // must remove an AI ('w') piece
        const cands = [];
        for (let i = 0; i < 24; i++) if (board[i] === 'w' && !(inMill(board, i, 'w') && countPieces(board, 'w') > 3)) cands.push(i);
        const ci = cands.length ? cands[Math.floor(Math.random() * cands.length)] : -1;
        if (ci < 0) return;
      ...
      if (phase === "place"){
        const empties = []; for (let i = 0; i < 24; i++) if (board[i] === '.') empties.push(i);
        if (!empties.length){ winner = "ai"; draw(); return; }
        ...
        const myPieces = []; for (let i = 0; i < 24; i++) if (board[i] === 'b') myPieces.push(i);
        const mvs = [];
        for (const i of myPieces) for (const j of ADJ[i]) if (board[j] === '.') mvs.push([i, j]);
        if (!mvs.length){ winner = "ai"; draw(); return; }
        const [from, to] = mvs[Math.floor(Math.random() * mvs.length)];
        board[to] = 'b'; board[from] = '.';
