// International draughts — 10×10 dark-squares-only. Men capture forwards AND
// backwards; flying kings (any distance along a diagonal); captures are
// compulsory but we simplify to picking any maximal capture chain (not
// strictly enforced). King promotion at last row. AI: captures-first random.
...
export function create(canvas) {
  ...
  function draw(){
    ...
    _fit(ctx, "International draughts (10×10) — captures mandatory; men capture backwards; flying kings", W/2, 18, W - 8);
    for (let r = 0; r < N; r++) for (let c = 0; c < N; c++){
